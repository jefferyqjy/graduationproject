 ...
 */
 ...
 function formValidation(isNeed) {
	 if(isNeed == true) {
		 var success = true;
		 var inputList = document.getElementsByTagName("input");
		 if(typeof inputList != "undefined" && inputList != null) {
			 ...
			 for(var i=0;i<len;i++) {
				 ...
				 if("text" == inputType || "password" == inputType || "select" == inputType) {
		 ...
		 if(isNotEmpty(option)) {  
			 if("M" == option) {
				 if(isEmpty(inputValue)) {
					 var id = input.name + "_errMsg";
					 var showEle = document.getElementById(id);
					 showEle.style.display = "block";
					 showEle.innerHTML="<font color='red'>该字段不能为空</font>";
					 return true;
				 }  else {
					 var id = input.name + "_errMsg";
					 var showEle = document.getElementById(id);
		...
		/* var maxLength = clazz.substr(7,4);
		 if(isNotEmpty(maxLength)) {
			 if(inputValue.length > maxLength) {
				 ...
				 var showEle = document.getElementById(id);
				 showEle.style.display = "block";
				 showEle.innerHTML="<font color='red'>该字段超出长度"+maxLength+"</font>";
				 return true;
			 }
		 }*/
	 }
 ...
 function checkNumber(input){
	 if(!isNaN(input.value)){ 
		...
		var showEle = document.getElementById(id);
		showEle.style.display = "block";
		showEle.innerHTML="<font color='red'>该字段必须为数字！</font>";
	    return true;
	} 	
 }
 
 
 	function commFireEvent(targetObj,eventName) {
		if(document.all) {
			targetObj.fireEvent(eventName);
		} else {
