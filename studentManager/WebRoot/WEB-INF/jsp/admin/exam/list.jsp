...
<html>
	<body>
		<display:table name="list"
			...
			<display:column title="学生">${row.student.studentName }</display:column>
			<display:column title="学号">${row.student.studentNo }</display:column>
			<display:column title="课程">${row.kc.kcName }</display:column>
			<display:column title="课程老师">${row.kc.skls }</display:column>
			<display:column title="考试名称">${row.examName }</display:column>
		<display:column title="考试时间">${row.examDate }</display:column>
		<display:column title="监考老师">${row.jkls }</display:column>
		<display:column title="分数">${row.code }</display:column>
