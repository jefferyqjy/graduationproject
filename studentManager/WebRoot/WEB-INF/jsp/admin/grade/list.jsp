...
<body>
	<display:table name="list" requestURI="list.html" class="list" id="row"
		...
		<display:column title="年级名称">${row.gradeName }</display:column>
		<display:column title="班级名称">${row.className }</display:column>
		<display:column title="专业名称">${row.majorName }</display:column>
		<display:column title="年级人数">${row.studentNum }</display:column>
